...
extract_column_names_first_step = re.findall('(where| AND| WHERE) (\w+)', query)
extract_main_table_name_first_step =  re.findall('FROM (\w+)', query)
extract_join_table_names_first_step = re.findall('JOIN (\w+)', query_multitable_join)
...
def extract_joined_table_names(extract_join_table_names_first_step):
    for result in extract_join_table_names_first_step:
        joined_table_names.append(result)
        
    print(joined_table_names)

extract_joined_table_names(extract_join_table_names_first_step)
